...
Given a set of $n + 1$ data points, $(x_0, f(x_0)), \ldots, (x_n, f(x_n))$, we would like to interpolate $f$. We consider the elementary Lagrange polynomials
...
We want to compute the value of Lagrange's interpolation polynomial, 
...
function LagrangeInterpolation(IntP,x0)
# Initialization
# Suppose that the n data points, are given in a 2 x n matrix, IntP.
X = IntP[1,:]  
Y = IntP[2,:]
n = size(IntP)[2]
polynomial_value = 0 ;
...
for i=1:n 
	# Initialization     
	  lagrange_frac = 1 ; #of lagrange polynomial
	# Main process  
	  for j=1:n #calculation of i-th lagrange polynomial
	     if j!=i
	       lagrange_frac = lagrange_frac * (x0-X[j])/(X[i]-X[j]) 
	     end
	  end
	  polynomial_value = polynomial_value + Y[i] * lagrange_frac; #calculation of p
	end
	return(polynomial_value)
end

# ╔═╡ 374dd2cd-7a86-4ccd-af1f-df3c7ef717fa
md"""
Now we are going to use the code above to interpolate a given $f$.
"""

# ╔═╡ 6aab085f-3b1d-490e-a801-f68d9f216bb1
md"""
### Action🎬
"""
...
@bind f_points html"""<input placeholder="40" type="number" min="0" step="1" style="border: none;
  appearance: none;
  background: #f2f2f2;
  padding: 12px;
  color:black;
...
begin
	...
	# Generate even partition of [a,b]
		f_x = collect(range(a, stop=b, length=f_points))
	#
	f_y = f.(f_x)
	#Plotting
	plot(f_x, f_y, title="Function f graph", linewidth=1, label="function f")
	xlabel!("x")
	ylabel!("y")
end
...
begin
	...
	plot(Lagrange[1,:],Lagrange[2,:], title="Lagrange Interpolation graph", label="Lagrange polynomial",linewidth=1)
end	

# ╔═╡ a84b9394-85eb-449f-9a57-0969c861e8b1
md"""
### Precision & Error of Lagrange Interpolation
"""

# ╔═╡ b60a5e4c-c7a6-4787-853e-369e3a9dd871
begin
	plot(f_x, f_y, label="Function f", linewidth=1)
	plot!(Lagrange[1,:], Lagrange[2,:], label="Lagrange", linewidth=1)
	plot!(title="F vs Lagrange Interpolation graph")
end	

# ╔═╡ 87921d5e-4d2b-43e7-a218-2921d6894f97
begin
	difference1 = abs.(f.(Lagrange[1, :]) .- Lagrange[2, :])
	plot(Lagrange[1, :], difference1,label= "Abs. error",title="Absolute difference of f-Lagrange")
end
...
function NewtonInterpolation(IntP,z0) 
	n = size(IntP)[2]
	DivDiff = zeros(n,n)
	DivDiff[:,1]= IntP[2,:] #1st column value of f given x-coordinates
	for j=2:n #filling of divided differences matrix
		for i=j:n
...
#Computation of value of p at z_0	
	X = z0 .- IntP[1,:]#[z0-x_1, z0-x_2,z0-x_3,...,z0-x_n]
	newton_y = a_coefficients[1]; 
#Main process
	for j = 2:n
		newton_y = newton_y + a_coefficients[j]*prod(X[1:j-1]);
	end		
	return(newton_y)
end

# ╔═╡ cb1a0d5f-ebe1-44de-bc9a-16b348f4eca1
md"""
### Action🎬
"""
...
begin
	if match_styles
		...
		Show(MIME"text/html"(),"""
		<p  style="font-size:15.0pt">
		We will be using $newton_points  points to calculate the interpolation using Lagrange and Newton method (of course the interpolation polynomial is the same, as it can be seen).
		</p>""")
	else
...
# ╔═╡ 36473c2f-bcff-4128-806b-45b8a21386c6
md"""
### Precision & Error of Newton Interpolation
"""

# ╔═╡ 782108f9-20e3-4ffe-95bd-dc44d8f483b9
begin
	difference2 = abs.(f.(Newton[1, :]) .- Newton[2, :])
	plot(Newton[1, :], difference2, xlabel = "x", ylabel = "", label="Abs error",title="Absolute difference of f-Newton")
end
