...
Show(MIME"text/html"(),"""<p style="font-size:14.0pt">
		Pick the first number (it should be ∈(0,1)):
		</p>""")

# ╔═╡ 75cc1e07-6129-4d4d-b3a2-ebd70e482992
@bind first_number TextField(default="0.1")

# ╔═╡ 53d58d7a-1880-4a0c-87fe-6785e6fc19e5
Show(MIME"text/html"(),"""<p style="font-size:14.0pt">
		Now pick the second number (it should be ∈(0,1)):
		</p>""")

# ╔═╡ 99132992-de52-4855-82dd-7e94b748da56
@bind second_number TextField(default="0.2")
...
begin #we want a<b
	...
	Show(MIME"text/html"(),"""<p style="font-size:17.0pt">
		Let the 🪄 name 'a' the smaller of the 2 and 'b' the bigger number!
		</p>""")
end
...
begin
	...
	if flag_a ==1
		with_terminal(print,"Fractional part of a=$a: $fractional_a (truncated)")
	else
		with_terminal(print,"Fractional part of a=$b: $fractional_a (not truncated)")
	end
end	
...
begin
	...
	if flag_b ==1
		with_terminal(print,"Fractional part of b=$b: $fractional_b (truncated)")
	else
		with_terminal(print,"Fractional part of b=$b: $fractional_b (not truncated)")
	end
end	
...
starting, ending = fixing_truncation_problems(fractional_a, flag_a, fractional_b, flag_b)
	...
	ending_dec = binary_to_decimal(ending)
		Show(MIME"text/html"(),"""
		<h6 style="font-size:16.0pt">So the starting and the ending number are: </h6> <br>
		<p  style="font-size:14.0pt">
		Starting number in </i>decimal</i> $starting_dec <br>
		Starting number in </i>binary</i> : $starting    <br>
		Ending number in </i>decimal</i>  : $ending_dec  <br>
		Ending number in </i>binary</i>   : $ending 
		</p>""")
end
