...
# ╔═╡ b962ebed-0191-4c4d-8769-17e9ae00a2a1
function output_data(n, b, integer_vector, fractional_vector, flag)
	if flag & 1 ==1
		print("Unfortunately the number $n was too long to be saved in a 24-bits word")
	else 
		print("The number $n in $b-adic in a 24-bit is represented as:")
		print(n<0 ? "-" : "")
		for i in integer_vector
			print(i)
		end
		if fractional_vector != [0]
			print(".")
			for i in fractional_vector
				print(i)
			end
		end
	end
	if flag & 2 == 2
		print("\n Caution")
		print("The number $n has been truncated in order to fit in a 24-bit word. Risk of errors!")
	end
end
...
#Fractional Part
	fractional_vector, fractional_part = fractional_conversion(fractional_part, integer_vector, digit_accur, b)  
#Setting flag for output data
	if  length(integer_vector) > 24 || (length(integer_vector) == 24 && fractional_part !=0)
		flag = flag | 1
	end
	if  length(integer_vector) < 24 && fractional_part !=0 
		flag = flag | 2
	end
	
	output_data(n, b, integer_vector, fractional_vector, flag)
end 


# ╔═╡ 2ee8cbfa-2b2f-44a4-9b1a-4f90cb7b830d
dec_to_b_ary(0.1,  2, 24)
